...
public class Main {
	public static void main(String[] args) {
		...
		System.out.println("\nСтатистика перевезень по водіях");
		carBase.getProductCountByDriver().forEach((name, count) ->
				System.out.println(name + ": " + count + " перевезень"));
		
		System.out.println("\nСтатистика перевезень за пунктами призначення");
		carBase.getProductCountByDestination().forEach((destination, count) ->
				System.out.println(destination + ": " + count + " перевезень"));
		
		System.out.println("\nТоп-водії за заробітком");
		carBase.topEarners(2).forEach(driver ->
				System.out.println(driver.getName() + ": заробіток = " + driver.getMoney() + " грн"));
	}
}